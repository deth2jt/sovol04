...
void Temperature::tick() {
  ...
  static uint8_t pwm_count1 = _BV(SOFT_PWM_SCALE-7);

  // avoid multiple loads of pwm_count
  uint8_t pwm_count_tmp = pwm_count;
  uint8_t pwm_count_tmp1 = pwm_count1;
    ...
    if (pwm_count_tmp1 >= 127) {
      ...
      #define _PWM_LOW(N,S) do{ if (S.count <= pwm_count_tmp1) WRITE_HEATER_##N(LOW); }while(0)
      #if HAS_HOTEND
    ...
    pwm_count1 = pwm_count_tmp1 + _BV(SOFT_PWM_SCALE-7);
